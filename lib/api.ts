...
export interface BlueprintOverrides {
  image_prompt?: string;
  aspect_ratio?: string;
  // Add other overrideable fields here
}
...
export async function getLatestRunForProject(
  ...
  if (!list.length && projectId === 'AjKHUnbF') {
    // If no runs for demo project, create one with a calendar entry
    const runId = nanoid(10);
    const demoEntry: CalendarEntry = {
      id: nanoid(8),
      date: '2025-12-31',
      channel: 'Instagram',
      type: 'Post',
      title: 'Demo New Year Post',
      description: 'A mock post for New Year created by ensureDemoProjects logic.'
    };
    runs[runId] = {
      runId,
      projectId,
      createdAt: new Date().toISOString(),
      results: {},
      calendar: { '2025-12-31': [demoEntry] },
    };
    write(LS_RUNS, runs);
    return runs[runId];
  }
  if (!list.length) return null;
  const latestRun = list.sort(
    (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  )[0]!;

  // Also ensure demo project has at least one entry for 2025-12-31 if missing
  if (projectId === 'AjKHUnbF' && (!latestRun.calendar || !latestRun.calendar['2025-12-31'])) {
    const demoEntry: CalendarEntry = {
      id: nanoid(8),
      date: '2025-12-31',
      channel: 'Instagram',
      type: 'Post',
      title: 'Demo New Year Post',
      description: 'A mock post for New Year added to existing run.'
    };
    latestRun.calendar = { ...latestRun.calendar, '2025-12-31': [demoEntry] };
    write(LS_RUNS, runs);
  }
  return latestRun;
}
...
async function simulateGeneration(versionId: string, entryId: string, projectId: string) {
  // Simulate backend processing time then update status to completed
  await new Promise(r => setTimeout(r, 4000));

  const store = read<Record<string, AssetVersion[]>>(LS_ASSET_VERSIONS, {});
  // Ideally we use projectId from context, but for mock we might guess it if not passed.
  // Actually, we pass projectId to this function.
  const key = versionKey(projectId, entryId);
...
export async function generateAsset(
  ...
  // Mock Implementation
  const projects = read<Record<string, ProjectMeta>>(LS_PROJECTS, {});
  const pid = Object.keys(projects)[0] || "demo-project";
  ...
  // We assume the assetId matches entryId for mocks
  const key = versionKey(pid, assetId);
  const store = read<Record<string, AssetVersion[]>>(LS_ASSET_VERSIONS, {});
  const list = store[key] || [];
  store[key] = [...list, v];
  write(LS_ASSET_VERSIONS, store);

  simulateGeneration(v.id, assetId, pid);

  return {
    version_id: v.id,
    task_id: "mock-task-" + nanoid(),
    status_url: "mock://status"
  };
}

export async function editAsset(
  assetId: string,
  request: EditAssetRequest
  ...
  const source = list.find(x => x.id === request.source_version_id);
...
export async function getVersionStatus(versionId: string): Promise<AssetVersion> {
  ...
  for (const key in store) {
    const found = store[key].find(v => v.id === versionId);
    if (found) return found;
  }
  throw new Error("Version not found in mock store");
}
...
export async function getAssetHistory(assetId: string): Promise<AssetVersion[]> {
  ...
  // For now, in mock mode, we might fallback to empty or rely on the old getAssetVersions signature.
  return [];
}
